...
def can_resynchronize_model(model: Union[Player, PlayerOwnedGame, Game], rate_limit: int = 60):
    ...
    delta = (
        (timezone.now() - model.achievements_resynchronized) if model.achievements_resynchronized is not None else -1
    )
    if not model.achievements_resynchronization_required and delta.seconds < rate_limit:
